...
/*   Updated: 2022/10/29 08:32:14 by hchahid          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	(void)arg;
	if (!ft_strncmp("-n", splited[1], ft_strlen(splited[1]) +  2) && !skip_echo_option(splited[1]))
...
void	echo_with_n(char **splited, t_env **env_p)
{
	...
	i = 1;
	while(!ft_strncmp("-n", splited[i], ft_strlen(splited[i]) +  2) || !skip_echo_option(splited[i]))
			i++;
	if (!splited[i])
		return ;
	while (splited[i])
...
void	write_e_var(t_env **env_p, char *my_var)
{
	...
	while (iter)
	{
		if (!ft_strncmp(my_var, iter->e_name, ft_strlen (my_var) + ft_strlen(iter->e_name)))
		{
			// printf("%s", (iter->e_var) + i);
